...
import { AlignCenter, AlignLeft, AlignRight, ArrowLeft, Check, ChevronDown, Download, ImagePlus, LoaderCircle, Plus, Search, Trash2, Type, Video } from "lucide-react"
import { api } from "./api"
import { uid, useEditor } from "./store"
import type { ImageElement, Slide, SlideElement, Song, TextElement, VideoElement } from "./types"

const WIDTH = 1920, HEIGHT = 1080

function Toolbar({ onImage }: { onImage: (file: File) => void }) {
...
function SongPanel({ addVideo }: { addVideo: (song: Song) => void }) {
  ...
  async function download(song: Song) { setBusy(song.id); setStatus("Preparando descarga…"); setError(""); try { const { jobId } = await api.downloadSong(song.id); await new Promise<void>((resolve, reject) => { const events = new EventSource(`/api/jobs/${jobId}/events`); events.onmessage = (event) => { const data = JSON.parse(event.data); setStatus(data.message); if (data.done) { events.close(); data.error ? reject(new Error(data.error)) : resolve() } }; events.onerror = () => { events.close(); reject(new Error("Se perdió la conexión con la descarga.")) } }); const fresh = await api.songs(); setSongs(fresh); const ready = fresh.find((item) => item.id === song.id); if (ready) addVideo(ready) } catch (e) { setError(e instanceof Error ? e.message : "No se pudo descargar el video.") } finally { setBusy(""); setStatus("") } }
...
export function Editor({ id, onBack }: { id: string; onBack: () => void }) {
  const { project, slideId, dirty, setProject, addElement, addSlide, selectSlide, markSaved, deleteElement, duplicateSlide, deleteSlide } = useEditor()
  const [loading, setLoading] = useState(true); const [error, setError] = useState(""); const [exporting, setExporting] = useState(false)
