...
function Admin() {
  ...
  const [model, setModel] = useState("openai/gpt-5.6-luna"); const [modelMessage, setModelMessage] = useState(""); const [modelBusy, setModelBusy] = useState(false)
  useEffect(() => { api.aiSettings().then((result) => setModel(result.model)).catch(() => undefined) }, [])
  ...
  async function saveModel(e: React.FormEvent) { e.preventDefault(); setModelMessage(""); setModelBusy(true); try { const result = await api.saveAISettings(model); setModel(result.model); setModelMessage("Modelo guardado.") } catch (reason) { setModelMessage(reason instanceof Error ? reason.message : "No se pudo guardar el modelo.") } finally { setModelBusy(false) } }
  return <main className="admin-shell"><button className="back-link" onClick={() => navigate("/")}><ArrowLeft size={18}/> Presentaciones</button><div className="admin-grid"><section className="admin-card"><p className="eyebrow">ADMINISTRACIÓN</p><h1>Crear usuario</h1><p className="muted">Dale acceso a otra persona. Podrá usar la aplicación en su propio equipo.</p><form onSubmit={submit}><label>Usuario<input value={username} onChange={(e) => setUsername(e.target.value)} required minLength={3}/></label><label>Contraseña<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={8}/></label><label>Rol<select value={role} onChange={(e) => setRole(e.target.value)}><option value="normal">Normal</option><option value="admin">Administrador</option></select></label>{message && <p className="success-box">{message}</p>}{error && <p className="form-error">{error}</p>}<button className="primary" disabled={!username || password.length < 8}>Crear usuario</button></form></section><section className="admin-card"><p className="eyebrow">INTELIGENCIA ARTIFICIAL</p><h1>Modelo</h1><p className="muted">Escribe el identificador exacto de Vercel AI Gateway.</p><form onSubmit={saveModel}><label>Proveedor y modelo<input value={model} onChange={(e) => setModel(e.target.value)} placeholder="openai/gpt-5.6-luna" required /></label><small className="field-help">Ejemplo: openai/gpt-5.6-luna</small>{modelMessage && <p className={modelMessage === "Modelo guardado." ? "success-box" : "form-error"}>{modelMessage}</p>}<button className="primary" disabled={modelBusy || !model.includes("/")}>{modelBusy ? "Guardando…" : "Guardar modelo"}</button></form></section></div></main>
}
